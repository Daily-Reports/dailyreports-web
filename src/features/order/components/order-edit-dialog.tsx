...
const OrderEditDialog: React.FC<OrderEditDialogProps> = ({orderId, open, setOpen, onEdit}) => {
    ...
    return (
        <Dialog open={open} onClose={clearFormAndClose}>
            <div className="text-center w-112">
                ...
                <form className="mt-3" onKeyDown={e => e.key === 'Enter' && e.preventDefault()}>
                    <InputValue title="Technical" value={technical} setValue={setTechnical}/>
                    <InputValue title="Description" value={description} setValue={setDescription}/>

                    <EventSelect events={events ?? []} selectValue={event} setSelectedValue={setEvent}/>

                    <div className="flex justify-between">
                        <AreaSelect areas={areas ?? []} selectValue={area} setSelectValue={setArea}/>
                        <SubareaSelect subareas={subareas ?? []} selectValue={subarea} setSelectedValue={setSubarea}/>
                    </div>

                    <OrderSpecialitySelect selectValue={speciality} setSelectedValue={setSpeciality}/>
                </form>
